/* Variables */;
var apiUrl = SiteRoot + 'perusahaan';
...
$(document).ready(function() {
	...
	$("input[name=logo]").enterKey(function (e) {
		e.preventDefault();
		$(".btn_save").click();
	});
	...
	table = $("#datatable-responsive").DataTable({
		...
		"buttons": [
			...
			},
			{
				...
				title: function () { return 'Perusahaan';},
				exportOptions: {
					/* Show column */
				...
				if (json.success === true) {
					...
					for (var i = 0; i < json.message.data.length; i++) {
						return_data.push({
							...
							2: json.message.data[i].perusahaan,
							3: json.message.data[i].logo,
						})
					}
					return return_data;
				} else {
	...
	$(tbl).on( 'click', '#btEdit', function () {
		...
		/* Set Edit Form Value */
		$("input[name=perusahaan]").val(data[2]);
		$("input[name=logo]").val(data[3]);
...
function btn_add() {
	...
	$('.modal-title').html('New Perusahaan');
