...
$(document).ready(function() {
				...
				if (json.success === true) {
					...
					for (var i = 0; i < json.message.data.length; i++) {
						return_data.push({
							...
							2: json.message.data[i].idperusahaan,
							3: json.message.data[i].bidang,
						})
					}
					return return_data;
				} else {
	...
	$(tbl).on( 'click', '#btEdit', function () {
		...
		/* Set Edit Form Value */
		$("input[name=idperusahaan]").val(data[2]);
		$("input[name=bidang]").val(data[3]);
		$('.btn_save').html('<i class="fa fa-save"></i> Update');
		$('.modal-title').html('Edit Bidang');
