...
$(document).ready(function() {
	...
	$("input[name=foto]").enterKey(function (e) {
		e.preventDefault();
		$(".btn_save").click();
	});
	...
	table = $("#datatable-responsive").DataTable({
		...
		"buttons": [
			...
			},
			{
				...
				title: function () { return 'Transaksi';},
				exportOptions: {
					/* Show column */
			...
			"dataSrc": function(json) {
				...
				if (json.success === true) {
					...
					for (var i = 0; i < json.message.data.length; i++) {
						return_data.push({
							0: json.message.data[i].id,
							1: json.message.data[i].no,
							2: json.message.data[i].iduser,
							3: json.message.data[i].foto,
							4: json.message.data[i].lat,
							5: json.message.data[i].lon,
							6: json.message.data[i].timestamp,
						})
					}
					return return_data;
				} else {
	...
	$(tbl).on( 'click', '#btEdit', function () {
		...
		/* Set Edit Form Value */
		$("input[name=iduser]").val(data[2]);
		$("input[name=foto]").val(data[3]);
		$("input[name=lat]").val(data[4]);
		$("input[name=lon]").val(data[5]);
		$("input[name=timestamp]").val(data[6]);
...
function btn_add() {
	...
	$('.modal-title').html('New Transaksi');
